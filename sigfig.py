...
def main():
    for line in data.strip().split('\n'):
        ...
        s = SigFig(inp)
        parts = []
        if s.sigfigs != sigfigs:
            parts.append('sigfigs %d (expected %d)' % (s.sigfigs, sigfigs))
        if s.least_significant_place != lsp:
            parts.append('lsp %d (expected %d)' % (s.least_significant_place, lsp))
        if str(s) != rep:
            parts.append('rep %r (expected %r)' % (str(s), rep))
        if not parts:
            continue
        print s.as_scientific().as_tuple()
        print '%s: %s' % (inp, ', '.join(parts))
        print
        #return

#op = SigFig('2000').shift_sigfigs(2)
#print op
#print op.as_tuple()
#print SigFig('2.00e3').as_tuple()
